...
async function publishEvent(detailType, detail) {
  try { console.log('publishEvent (noop):', detailType, detail); } catch (e) { console.error(e); }
}
...
exports.getProducts = async () => {
  try {
    ...
    console.error(err);
    if (process.env.DEBUG === 'true') return buildResponse(500, { message: 'Server error', error: err.message, stack: err.stack });
...
exports.createProduct = async (event) => {
  try {
    ...
    try { jwt.verify(token, process.env.JWT_SECRET || 'change-me'); } catch (err) { return buildResponse(401, { message: 'Invalid token' }); }
    ...
    const pool = await getPool();
    const [result] = await pool.query('INSERT INTO products (name, description, price, image, stock) VALUES (?, ?, ?, ?, ?)', [name, description || null, price, image || null, stock || 0]);
...
exports.updateProduct = async (event) => {
  ...
  try {
    ...
    ['name','description','price','image','stock'].forEach(k => { if (body[k] !== undefined) { fields.push(`${k} = ?`); values.push(body[k]); } });
